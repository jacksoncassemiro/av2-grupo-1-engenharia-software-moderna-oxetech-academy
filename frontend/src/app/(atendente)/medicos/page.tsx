...
import { validarEmailObrigatorio, validarNomeCompleto, validarTextoMinimo } from '@/lib/validadores';
    ...
    async function carregar(especialidadeId?: string | null) {
        ...
        try {
            ...
            const [listaMedicos, listaEspecialidades] = await Promise.all([
                ...
                especialidades.length === 0 ? api<Especialidade[]>('/especialidades') : Promise.resolve(especialidades),
            ]);
                ...
                <form onSubmit={form.onSubmit(cadastrar)}>
                    <Stack gap="md">
                        <Group grow align="flex-start" wrap="wrap">
                            ...
                            />
                            <TextInput label="CRM" placeholder="CRM12345" withAsterisk {...form.getInputProps('crm')} />
                        </Group>
                        <Group grow align="flex-start" wrap="wrap">
                            <TextInput label="Nome" placeholder="Dra. Ana Souza" withAsterisk {...form.getInputProps('nome')} />
                            <TextInput
                                label="E-mail"
                        ...
                        <Table.Tbody>
                            {medicos.map((medico) => (
                                <Table.Tr key={medico.id}>
                                    ...
                                    <Table.Td visibleFrom="sm">{nomeDaEspecialidade(medico.especialidade_id)}</Table.Td>
                                    <Table.Td>

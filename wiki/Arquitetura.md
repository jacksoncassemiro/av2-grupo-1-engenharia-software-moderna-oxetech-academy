...
> repositório (monorepo — [ADR-001](ADR-001-Monorepo)), mas são **processos separados,
...
> no [ADR-008](ADR-008-Rebaseline-de-Escopo) §3.
...
| **Repository** | `backend/app/repositories/` | Isola o acesso a dados. É o que permite testar Service sem subir banco — ver [`06-design-patterns.md`](Design-Patterns) |
...
| ADR | Decisão |
|---|---|
| [ADR-001](ADR-001-Monorepo) | Monorepo com `backend/` e `frontend/` |
| [ADR-002](ADR-002-Camadas-MVC) | Camadas Controller → Service → Repository → Model |
| [ADR-003](ADR-003-Autenticacao) | Login flexível CPF ou e-mail com JWT |
| [ADR-004](ADR-004-Bootstrap-do-Atendente) | Seed idempotente + RN14 |
| [ADR-005](ADR-005-Cadastro-de-Paciente) | Auto-cadastro e cadastro por atendente convergindo |
| [ADR-006](ADR-006-Design-Patterns) | Strategy + Repository; índice parcial para a RN03 |
| [ADR-007](ADR-007-Vitest) | Vitest no Next.js em vez de Jest |
| [ADR-008](ADR-008-Rebaseline-de-Escopo) | Rebaseline de escopo e separação backend × frontend na documentação |
